...
## Why documentation and screenshots appear later

The repository history shows that a large portion of the runnable system landed before the later README, screenshot, and authentication-guidance updates. That sequence is intentional.

The implementation was stabilized first. Once authenticated browser workflows, conflict handling, and current UI states could be exercised reliably, the repository documentation was updated to match the verified behavior and the current application surface.

In other words, the later documentation work was primarily alignment and evidence capture for an already implemented application baseline, not a change in product direction.
